"""
Action generation module for the Soko Solver.
Generates valid successor states based on applicable actions.
"""
...
def can_move_to(maze: Maze, state: State, new_pos: Tuple[int, int]) -> bool:
    """
    Check if robot can move to a position (no wall, no locked door).
    """
...
def move_robot(state: State, new_pos: Tuple[int, int], action_name: str) -> State:
    """
...
def take_key(state: State, key_id: str) -> State:
    """
    Take a key that's on the floor at the robot's location.
    """
...
def lift_box(state: State, box_id: str) -> State:
    """
    Lift a box at the robot's location.
    """
    return State(
        robot_pos=state.robot_pos,
        carried_box=box_id,
...
def get_successors(maze: Maze, state: State) -> List[Tuple[State, str]]:
    ...
    if state.carried_box is not None:
        ...
        if zone_pos and state.robot_pos == zone_pos:
